...
## Progressive Disclosure（文档写作铁律）

**省 token = 省钱 = 快。** 每个文件必须让 agent 用最少的阅读量做出正确决策。

### 三条规则

**1. 信息只住一个地方（Single Source of Truth）**
- 同一个事实只在一个文件里写完整版
- 其他地方只写一句摘要 + `> 详见 XXX.md`
- 违反检查：如果你在两个文件里看到同一张表 / 同一段细节 → 删一个，留引用

**2. 每个文件开头声明"什么时候才读"**
- 格式：`> **什么时候读：** [触发条件]。日常路由不需要读。`
- BOOTSTRAP.md 的 Layer 0 索引表已经给出了每个文件的触发条件
- 新建文件必须加这一行，否则违反规则

**3. BOOTSTRAP.md 是唯一入口**
- Agent 启动只读 BOOTSTRAP.md + TASKS.md
- BOOTSTRAP 的 Layer 0 表包含每个文件的一句话摘要
- 新建文件 → 先更新 BOOTSTRAP.md 的 Layer 0 表，再写文件内容
- 如果一句话摘要写不出来 → 这个文件不该独立存在，合并到别的文件里

### 违反了怎么办

发现重复内容或 agent 读了不需要的文件 → 写入 Learning Log：
```
- [日期] Progressive Disclosure 违反：[哪里重复了] → [怎么修的]
```
...
## Journey 定义（= 助理会计季度末工作流）

> 完整背景见 `CONTEXT.md`。每个 Journey 用什么 tools 见 `TOOLS_STRATEGY.md`。
> **当前状态/进度只看 `TASKS.md`，不在这里维护。**

| Journey | 助理会计在做什么 | AI 证明什么 |
|---------|-----------------|-----------|
| **J1** | "帮我对一下 Q1 的账" | 读数据 + 逐笔匹配 + 找出所有不匹配 |
| **J2** | "这几笔对不上的，查一下什么情况" | 追查异常原因，给出会计判断和处理建议 |
| **J3** | "算一下 GST，我要报 IRD" | 正确计算 NZ 15% GST，算出净额 |
| **J4** | "出一份 Q1 的 P&L 给我" | 按科目汇总，生成标准损益表 |
| **J5** | "整理成底稿，每个数字要有来源" | 生成完整 Working Paper，数字可溯源 |
| **J6** | 合伙人审阅 + 追问细节 | 上传文件、看结果、自然语言问答 |
