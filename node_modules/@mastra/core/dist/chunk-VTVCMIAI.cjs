...
var chunkQKQGKEN7_cjs = require('./chunk-QKQGKEN7.cjs');
var chunkKZ4IKNPN_cjs = require('./chunk-KZ4IKNPN.cjs');
...
var chunkN3ROEJG4_cjs = require('./chunk-N3ROEJG4.cjs');
var chunkXNWF6CYR_cjs = require('./chunk-XNWF6CYR.cjs');
var chunk6BJ7XHRH_cjs = require('./chunk-6BJ7XHRH.cjs');
var chunkOQDRPRKM_cjs = require('./chunk-OQDRPRKM.cjs');
var chunkTGUDI64A_cjs = require('./chunk-TGUDI64A.cjs');
...
var ProcessorRunner = class _ProcessorRunner {
  ...
  async runOutputProcessors(messageList, tracingContext, requestContext, retryCount = 0, writer) {
    for (const [index, processorOrWorkflow] of this.outputProcessors.entries()) {
      ...
      if (result instanceof chunkZXOWG32X_cjs.MessageList) {
  ...
  static async validateAndFormatProcessInputStepResult(result, {
    ...
    if (result instanceof chunkZXOWG32X_cjs.MessageList) {
      ...
      if (result.model) {
        const resolvedModel = await chunkN3ROEJG4_cjs.resolveModelConfig(result.model);
            ...
            case "finish":
              ...
              try {
                if (self.processorRunner && !self.#options.isLLMExecutionStep) {
                  ...
                  const outputText = lastResponseMessage ? chunkZXOWG32X_cjs.coreContentToString(lastResponseMessage.content) : "";
  ...
  switch (chunk.type) {
    ...
    case "file":
      if (mode === "generate") {
        return {
          type: "file",
          file: new chunkZXOWG32X_cjs.DefaultGeneratedFile({
      ...
      return {
        type: "file",
        file: new chunkZXOWG32X_cjs.DefaultGeneratedFileWithType({
          data: chunk.payload.data,
          mediaType: chunk.payload.mimeType
        })
      };
...
async function validateStepInput({
  ...
  let validationError;
  if (validateInputs && chunkTGUDI64A_cjs.isZodType(step.inputSchema)) {
...
async function validateStepResumeData({ resumeData, step }) {
  ...
  if (resumeSchema && chunkTGUDI64A_cjs.isZodType(resumeSchema)) {
...
async function validateStepSuspendData({
  ...
  if (suspendSchema && validateInputs && chunkTGUDI64A_cjs.isZodType(suspendSchema)) {
...
async function validateStepStateData({
  ...
  if (stateSchema && validateInputs && chunkTGUDI64A_cjs.isZodType(stateSchema)) {
...
async function validateStepRequestContext({
  ...
  if (requestContextSchema && validateInputs && chunkTGUDI64A_cjs.isZodType(requestContextSchema)) {
...
var createTimeTravelExecutionParams = (params) => {
  ...
  for (const [index, entry] of graph.steps.entries()) {
    ...
    stepIds.forEach((stepId) => {
      ...
      if (result) {
        const formattedResult = chunkTGUDI64A_cjs.removeUndefinedValues(result);
  ...
  do {
    ...
    const evalSpan = await engine.createChildSpan({
      ...
      options: {
        ...
        input: chunkTGUDI64A_cjs.selectFields(result.output, ["stepResult", "output.text", "output.object", "messages"]),
        attributes: {
...
function isToolStep(input) {
  return input instanceof chunkU3HBG2GU_cjs.Tool;
}
function isStepParams(input) {
  return input !== null && typeof input === "object" && "id" in input && "execute" in input && !(input instanceof Agent) && !(input instanceof chunkU3HBG2GU_cjs.Tool);
}
  ...
  return {
    ...
    execute: async ({ inputData, requestContext, tracingContext, outputWriter }) => {
      ...
      const passThrough = {
        ...
        messageList: messageList ?? (Array.isArray(messages) ? new chunkZXOWG32X_cjs.MessageList().add(messages, "input").addSystem(systemMessages ?? []) : void 0),
        stepNumber,
        systemMessages,
        streamParts,
        state,
...
function isProcessor(obj) {
  return obj !== null && typeof obj === "object" && "id" in obj && typeof obj.id === "string" && !(obj instanceof Agent) && !(obj instanceof chunkU3HBG2GU_cjs.Tool) && (typeof obj.processInput === "function" || typeof obj.processInputStep === "function" || typeof obj.processOutputStream === "function" || typeof obj.processOutputResult === "function" || typeof obj.processOutputStep === "function");
}
...
var Run = class {
  ...
  async _validateInput(inputData) {
    ...
    if (this.validateInputs && this.inputSchema && chunkTGUDI64A_cjs.isZodType(this.inputSchema)) {
  ...
  async _validateInitialState(initialState) {
    ...
    if (this.validateInputs) {
      ...
      if (stateSchema && chunkTGUDI64A_cjs.isZodType(stateSchema)) {
  ...
  async _validateRequestContext(requestContext) {
    if (this.validateInputs && this.requestContextSchema && chunkTGUDI64A_cjs.isZodType(this.requestContextSchema)) {
      const contextValues = requestContext?.all ?? {};
...
` + errors.map((e) => `- ${e.path?.join(".")}: ${e.message}`).join("\n")
  ...
  async _validateResumeData(resumeData, suspendedStep) {
    ...
    if (suspendedStep && suspendedStep.resumeSchema && this.validateInputs && chunkTGUDI64A_cjs.isZodType(suspendedStep.resumeSchema)) {
  ...
  async _validateTimetravelInputData(inputData, step) {
    let inputDataToUse = inputData;
    if (step && step.inputSchema && this.validateInputs && chunkTGUDI64A_cjs.isZodType(step.inputSchema)) {
      const inputSchema = step.inputSchema;
...
var MastraScorer = class _MastraScorer {
  ...
  async executePromptStep(scorerStep, tracingContext, context) {
    ...
    const resolvedModel = await chunkN3ROEJG4_cjs.resolveModelConfig(modelConfig, void 0, this.#mastra);
        ...
        options: {
          ...
          onStepFinish: async (props) => {
            ...
            if (!isNaN(remainingTokens) && remainingTokens > 0 && remainingTokens < 2e3) {
              ...
              await chunkTGUDI64A_cjs.delay(10 * 1e3);
            }
          },
...
async function prepareMemoryStep({
  ...
  if (typeof messages === "string") {
    ...
    const messageList = new chunkZXOWG32X_cjs.MessageList({
      threadId: thread?.id,
      resourceId: thread?.resourceId
    });
...
${skillInstructions}`;
  ...
  createSkillActivateTool() {
    ...
    return chunkU3HBG2GU_cjs.createTool({
      id: "skill-activate",
  ...
  createSkillReadReferenceTool() {
    ...
    return chunkU3HBG2GU_cjs.createTool({
      ...
      execute: async ({ skillName, referencePath, startLine, endLine }) => {
        ...
        const result = chunkQKQGKEN7_cjs.extractLines(fullContent, startLine, endLine);
        return {
          success: true,
  ...
  __primitive({
    ...
    return {
      before: async () => {
        ...
        let messageList = new chunkZXOWG32X_cjs.MessageList({
          threadId,
          resourceId,
      ...
      after: async ({
        ...
        const messageListResponses = new chunkZXOWG32X_cjs.MessageList({
          threadId,
          resourceId,
          generateMessageId: this.capabilities.mastra?.generateId?.bind(this.capabilities.mastra),
          // @ts-expect-error Flag for agent network messages
...
var prepareMemoryStepOutputSchema = z10.z.object({
  ...
  messageList: z10.z.instanceof(chunkZXOWG32X_cjs.MessageList),
...
var Agent = class extends chunkRO47SMI7_cjs.MastraBase {
  ...
  async #validateRequestContext(requestContext) {
    if (this.#requestContextSchema && chunkTGUDI64A_cjs.isZodType(this.#requestContextSchema)) {
      const contextValues = requestContext?.all ?? {};
      const validatedRequestContext = await this.#requestContextSchema.safeParseAsync(contextValues);
      if (!validatedRequestContext.success) {
...
${errorMessages}`,
  ...
  listTools({ requestContext = new chunkVDKWYUGC_cjs.RequestContext() } = {}) {
    if (typeof this.#tools !== "function") {
      return chunkTGUDI64A_cjs.ensureToolProperties(this.#tools);
    }
    ...
    return resolveMaybePromise(result, (tools) => {
      ...
      return chunkTGUDI64A_cjs.ensureToolProperties(tools);
    });
  }
  /**
  ...
  getLLM({
    ...
    return resolveMaybePromise(modelToUse, (resolvedModel) => {
      ...
      if (isSupportedLanguageModel(resolvedModel)) {
        ...
        llm = new chunkXNWF6CYR_cjs.MastraLLMV1({
          model: resolvedModel,
          mastra: this.#mastra,
          options: { tracingPolicy: this.#options?.tracingPolicy }
        });
  ...
  async resolveModelConfig(modelConfig, requestContext) {
    try {
      return await chunkN3ROEJG4_cjs.resolveModelConfig(modelConfig, requestContext, this.#mastra);
    } catch (error) {
      const mastraError = new chunk4U7ZLI36_cjs.MastraError({
  ...
  async generateTitleFromUserMessage({
    ...
    const normMessage = new chunkZXOWG32X_cjs.MessageList().add(message, "user").get.all.ui().at(-1);
    if (!normMessage) {
    ...
    if (isSupportedLanguageModel(llm.getModel())) {
      const messageList = new chunkZXOWG32X_cjs.MessageList().add(
        [
          {
            role: "system",
  ...
  async genTitle(userMessage, requestContext, tracingContext, model, instructions) {
    try {
      if (userMessage) {
        const normMessage = new chunkZXOWG32X_cjs.MessageList().add(userMessage, "user").get.all.ui().at(-1);
        if (normMessage) {
  ...
  async listMemoryTools({
    ...
    if (memoryTools) {
      ...
      for (const [toolName, tool2] of Object.entries(memoryTools)) {
        ...
        const convertedToCoreTool = chunkTGUDI64A_cjs.makeCoreTool(toolObj, options, void 0, autoResumeSuspendedTools);
  ...
  async listWorkspaceTools({
    ...
    const workspaceTools = chunkQKQGKEN7_cjs.createWorkspaceTools(workspace);
  ...
  async listAssignedTools({
    ...
    const assignedCoreToolEntries = await Promise.all(
      assignedToolEntries.map(async ([k, tool2]) => {
        ...
        return [k, chunkTGUDI64A_cjs.makeCoreTool(tool2, options, void 0, autoResumeSuspendedTools)];
      })
    );
    ...
    if (toolsFromToolsets.length > 0) {
      ...
      for (const toolset of toolsFromToolsets) {
        for (const [toolName, tool2] of Object.entries(toolset)) {
          ...
          const convertedToCoreTool = chunkTGUDI64A_cjs.makeCoreTool(toolObj, options, "toolset", autoResumeSuspendedTools);
  ...
  async listClientTools({
    ...
    if (clientToolsForInput.length > 0) {
      ...
      for (const [toolName, tool2] of clientToolsForInput) {
        ...
        const convertedToCoreTool = chunkTGUDI64A_cjs.makeCoreTool(rest, options, "client-tool", autoResumeSuspendedTools);
        toolsForRequest[toolName] = convertedToCoreTool;
      }
    }
    return toolsForRequest;
  ...
  async listAgentTools({
    ...
    if (Object.keys(agents).length > 0) {
      for (const [agentName, agent] of Object.entries(agents)) {
        ...
        const toolObj = chunkU3HBG2GU_cjs.createTool({
          id: `agent-${agentName}`,
        ...
        convertedAgentTools[`agent-${agentName}`] = chunkTGUDI64A_cjs.makeCoreTool(
          toolObj,
          options,
          void 0,
          autoResumeSuspendedTools
  ...
  async listWorkflowTools({
    ...
    if (Object.keys(workflows).length > 0) {
      for (const [workflowName, workflow] of Object.entries(workflows)) {
        ...
        convertedWorkflowTools[`workflow-${workflowName}`] = chunkTGUDI64A_cjs.makeCoreTool(
          toolObj,
          options,
          void 0,
          autoResumeSuspendedTools
  ...
  async convertTools({
    ...
    if (this.#mastra) {
      mastraProxy = chunkTGUDI64A_cjs.createMastraProxy({ mastra: this.#mastra, logger });
    }
  ...
  async prepareModels(requestContext, model) {
    if (model || !Array.isArray(this.model)) {
      ...
      let headers;
      if (resolvedModel instanceof chunkN3ROEJG4_cjs.ModelRouterLanguageModel) {
    ...
    const models = await Promise.all(
      this.model.map(async (modelConfig) => {
        ...
        let headers;
        if (model2 instanceof chunkN3ROEJG4_cjs.ModelRouterLanguageModel) {
  ...
  async #execute({ methodType, resumeContext, ...options }) {
    ...
    if ("structuredOutput" in options && options.structuredOutput?.schema) {
      ...
      if (targetProvider.includes("openai") || targetModelId.includes("openai")) {
        if (chunkTGUDI64A_cjs.isZodType(options.structuredOutput.schema) && targetModelId) {
          const modelInfo = {
  ...
  async generate(messages, options) {
    ...
    const mergedOptions = chunkTGUDI64A_cjs.deepMerge(
      defaultOptions,
      options ?? {}
    );
  ...
  async resumeStream(resumeData, streamOptions) {
    ...
    let mergedStreamOptions = chunkTGUDI64A_cjs.deepMerge(
      defaultOptions,
      streamOptions ?? {}
    );
    const llm = await this.getLLM({
...
var ToolSearchProcessor = class {
  ...
  constructor(options) {
    ...
    this.bm25Index = new chunkQKQGKEN7_cjs.BM25Index({}, TOOL_SEARCH_TOKENIZE_OPTIONS);
    this.indexTools();
    if (this.ttl > 0) {
  ...
  async processInputStep(args) {
    ...
    const searchTool = chunkU3HBG2GU_cjs.createTool({
      id: "search_tools",
    ...
    const loadTool = chunkU3HBG2GU_cjs.createTool({
      id: "load_tool",
...
var SemanticRecall = class {
  ...
  formatCrossThreadMessages(messages, currentThreadId) {
    let result = ``;
    const v1Messages = new chunkZXOWG32X_cjs.MessageList().add(messages, "memory").get.all.v1();
    let lastYmd = null;
...
https://mastra.ai/en/docs/memory/semantic-recall`
      ...
      if (typeof config.embedder === "string") {
        this.embedder = new chunkN3ROEJG4_cjs.ModelRouterEmbeddingModel(config.embedder);
      } else {
...
https://mastra.ai/en/docs/memory/overview`
  ...
  setEmbedder(embedder, embedderOptions) {
    if (typeof embedder === "string") {
      this.embedder = new chunkN3ROEJG4_cjs.ModelRouterEmbeddingModel(embedder);
    } else {
  ...
  getMergedThreadConfig(config) {
    ...
    const mergedConfig = chunkTGUDI64A_cjs.deepMerge(this.threadConfig, config || {});
...
var MockMemory = class extends MastraMemory {
  constructor({
    ...
    super({
      name: "mock",
      storage: storage || new chunkKZ4IKNPN_cjs.InMemoryStore(),
      options: {
  ...
  listTools(_config) {
    ...
    return {
      updateWorkingMemory: chunkU3HBG2GU_cjs.createTool({
...
//# sourceMappingURL=chunk-VTVCMIAI.cjs.map
//# sourceMappingURL=chunk-VTVCMIAI.cjs.map