...
var chunkKH3G65IS_cjs = require('./chunk-KH3G65IS.cjs');
...
var Mastra = class {
  ...
  get datasets() {
    if (!this.#datasets) {
      this.#datasets = new chunkKH3G65IS_cjs.DatasetsManager(this);
    }
  ...
  constructor(config) {
    ...
    const workflowEventProcessor = new chunkHWG7NPJA_cjs.WorkflowEventProcessor({ mastra: this });
    ...
    if (storage) {
      storage = chunkVTVCMIAI_cjs.augmentWithInit(storage);
    }
    ...
    chunkMR7ZWBL6_cjs.registerHook("onScorerRun" /* ON_SCORER_RUN */, chunk6Q2UD3XF_cjs.createOnScorerHook(this));
  ...
  addAgent(agent, key, options) {
    ...
    let mastraAgent;
    if (chunk2IO5Q7OZ_cjs.isToolLoopAgentLike(agent)) {
      mastraAgent = chunk2IO5Q7OZ_cjs.toolLoopAgentToMastraAgent(agent, { fallbackName: key });
    } else {
      mastraAgent = agent;
    }
   ...
   */
  setStorage(storage) {
    this.#storage = chunkVTVCMIAI_cjs.augmentWithInit(storage);
  }
  setLogger({ logger }) {
    this.#logger = logger;
    if (this.#agents) {
  ...
  #syncGatewayRegistry() {
    try {
      ...
      import('./provider-registry-HMTFO5FV.cjs').then(async ({ GatewayRegistry }) => {
...
//# sourceMappingURL=chunk-VAKB5EXJ.cjs.map
//# sourceMappingURL=chunk-VAKB5EXJ.cjs.map