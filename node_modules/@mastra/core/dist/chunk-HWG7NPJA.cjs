...
var StepExecutor = class extends chunkRO47SMI7_cjs.MastraBase {
  ...
  async execute(params) {
    ...
    const { inputData, validationError } = await chunkVTVCMIAI_cjs.validateStepInput({
    ...
    try {
      ...
      const stepOutput = await step.execute(
        chunkVTVCMIAI_cjs.createDeprecationProxy(
          {
            ...
            getStepResult: chunkVTVCMIAI_cjs.getStepResult.bind(this, stepResults),
            suspend: async (suspendPayload, suspendOptions) => {
              const { suspendData, validationError: validationError2 } = await chunkVTVCMIAI_cjs.validateStepSuspendData({
          ...
          {
            paramName: "runCount",
            deprecationMessage: chunkVTVCMIAI_cjs.runCountDeprecationMessage,
            logger: this.logger
          }
        )
      );
      ...
      return {
        ...
        tripwire: error instanceof chunkVTVCMIAI_cjs.TripWire ? {
...
function createTripWireFromChunk(chunk) {
  const { payload } = chunk;
  return new chunkVTVCMIAI_cjs.TripWire(
  ...
  if (resumeData !== void 0 && forEachIndex === void 0 && currentResult?.output?.length > 0) {
    ...
    if (suspendedIndices.length > 0) {
      ...
      for (const suspIdx of indicesToResume) {
        updatedOutput[suspIdx] = chunkKZ4IKNPN_cjs.createPendingMarker();
      }
  ...
  async processWorkflowStepRun({
    ...
    if (step.step instanceof EventedWorkflow || step.step.component === "WORKFLOW") {
      if (resumeSteps?.length === 1 && resumeSteps[0] === step.step.id) {
        ...
        const timeTravelParams = chunkVTVCMIAI_cjs.createTimeTravelExecutionParams({
    ...
    const { resumeData: timeTravelResumeData, validationError: timeTravelResumeValidationError } = await chunkVTVCMIAI_cjs.validateStepResumeData({
...
function cloneWorkflow(workflow, opts) {
  const wf = new chunkVTVCMIAI_cjs.Workflow({
    id: opts.id,
...
function isAgent(input) {
  return input instanceof chunkVTVCMIAI_cjs.Agent;
}
function isToolStep(input) {
  return input instanceof chunkU3HBG2GU_cjs.Tool;
}
function isStepParams(input) {
  return input !== null && typeof input === "object" && "id" in input && "execute" in input && !(input instanceof chunkVTVCMIAI_cjs.Agent) && !(input instanceof chunkU3HBG2GU_cjs.Tool);
}
function isProcessor(obj) {
  return obj !== null && typeof obj === "object" && "id" in obj && typeof obj.id === "string" && !(obj instanceof chunkVTVCMIAI_cjs.Agent) && !(obj instanceof chunkU3HBG2GU_cjs.Tool) && (typeof obj.processInput === "function" || typeof obj.processInputStep === "function" || typeof obj.processOutputStream === "function" || typeof obj.processOutputResult === "function" || typeof obj.processOutputStep === "function");
}
...
function createStepFromAgent(params, agentOrToolOptions) {
  ...
  return {
    ...
    execute: async ({
      ...
      const isV2Model = chunkVTVCMIAI_cjs.isSupportedLanguageModel(modelInfo);
...
function createStepFromProcessor(processor) {
  ...
  return {
    ...
    inputSchema: chunkVTVCMIAI_cjs.ProcessorStepSchema,
    outputSchema: chunkVTVCMIAI_cjs.ProcessorStepOutputSchema,
    execute: async ({ inputData, requestContext, tracingContext }) => {
      ...
      const abort = (reason, options) => {
        throw new chunkVTVCMIAI_cjs.TripWire(reason || `Tripwire triggered by ${processor.id}`, options, processor.id);
      };
      ...
      const executePhaseWithSpan = async (fn) => {
        try {
          ...
          if (error instanceof chunkVTVCMIAI_cjs.TripWire) {
        ...
        switch (phase) {
          case "input": {
            if (processor.processInput) {
              ...
              if (result instanceof chunkZXOWG32X_cjs.MessageList) {
                ...
                chunkVTVCMIAI_cjs.ProcessorRunner.applyMessagesToMessageList(
                  result,
          ...
          case "inputStep": {
            if (processor.processInputStep) {
              ...
              const validatedResult = await chunkVTVCMIAI_cjs.ProcessorRunner.validateAndFormatProcessInputStepResult(result, {
                messageList: passThrough.messageList,
                processor,
                stepNumber: stepNumber ?? 0
              });
...
var EventedWorkflow = class extends chunkVTVCMIAI_cjs.Workflow {
  constructor(params) {
    super(params);
...
var EventedRun = class extends chunkVTVCMIAI_cjs.Run {
  constructor(params) {
    super(params);
  ...
  stream({
    ...
    this.streamOutput = new chunkVTVCMIAI_cjs.WorkflowRunOutput({
      runId: this.runId,
      workflowId: this.workflowId,
      stream
    });
...
var EventedExecutionEngine = class extends chunkVTVCMIAI_cjs.ExecutionEngine {
  eventProcessor;
  constructor({
    mastra,
    eventProcessor,
  ...
  async execute(params) {
    ...
    const finishCb = async (event, ack) => {
      ...
      if (["workflow.end", "workflow.fail", "workflow.suspend"].includes(event.type)) {
        ...
        if (event.type === "workflow.fail" && event.data.stepResults) {
          event.data.stepResults = chunkVTVCMIAI_cjs.hydrateSerializedStepErrors(event.data.stepResults);
        }
    ...
    for (const [stepId, stepResult] of Object.entries(stepResultsWithoutTopLevelState)) {
      cleanStepResults[stepId] = chunkVTVCMIAI_cjs.cleanStepResult(stepResult);
    }
    let callbackArg;
...
//# sourceMappingURL=chunk-HWG7NPJA.cjs.map
//# sourceMappingURL=chunk-HWG7NPJA.cjs.map