...
async function executeAgent(agent, item, signal) {
  ...
  const rawResult = chunkVTVCMIAI_cjs.isSupportedLanguageModel(model) ? await agent.generate(input, {
    scorers: {},
  ...
  const settled = await Promise.allSettled(
    scorers.map(async (scorer) => {
      ...
      if (storage && result.score !== null) {
        try {
          await chunk6Q2UD3XF_cjs.validateAndSaveScore(storage, {
            scorerId: scorer.id,
            score: result.score,
...
//# sourceMappingURL=chunk-KH3G65IS.cjs.map
//# sourceMappingURL=chunk-KH3G65IS.cjs.map