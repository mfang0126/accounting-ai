...
async function getWorkspaceInformation({
  ...
  const location = closestPkgJson ? path.dirname(chunkITC6JDLC_cjs.slash(closestPkgJson)) : chunkITC6JDLC_cjs.slash(process.cwd());
...
function getInputPlugins({ entry, isVirtualFile }, mastraEntry, { sourcemapEnabled }) {
  const normalizedMastraEntry = chunkITC6JDLC_cjs.slash(mastraEntry);
  ...
  plugins.push(
    ...[
      chunkE4SQOERA_cjs.tsConfigPaths(),
      {
        ...
        resolveId(id) {
          if (id === "#server") {
            return chunkITC6JDLC_cjs.slash(url.fileURLToPath(undefined("@mastra/deployer/server")));
          }
          if (id === "#mastra") {
      ...
      chunkE4SQOERA_cjs.removeDeployer(mastraEntry, {
...
async function captureDependenciesToOptimize(output, workspaceMap, projectRoot, initialDepsToOptimize, {
  ...
  if (!output.facadeModuleId.startsWith("\0virtual:")) {
    entryRootPath = await chunkE4SQOERA_cjs.getPackageRootPath(output.facadeModuleId) || projectRoot;
  }
  for (const [dependency, bindings] of Object.entries(output.importedBindings)) {
    if (isNodeBuiltin(dependency) || dependency.startsWith("#")) {
      continue;
    }
    const pkgName = chunkITC6JDLC_cjs.getPackageName(dependency);
    let rootPath = null;
    let isWorkspace = false;
    let version;
    if (pkgName) {
      rootPath = await chunkE4SQOERA_cjs.getPackageRootPath(dependency, entryRootPath);
    ...
    const normalizedRootPath = rootPath ? chunkITC6JDLC_cjs.slash(rootPath) : null;
  ...
  if (dynamicImports.length) {
    for (const dynamicImport of dynamicImports) {
      if (!depsToOptimize.has(dynamicImport) && !isNodeBuiltin(dynamicImport)) {
        const pkgName = chunkITC6JDLC_cjs.getPackageName(dynamicImport);
        let version;
        let rootPath = null;
        if (pkgName) {
          rootPath = await chunkE4SQOERA_cjs.getPackageRootPath(dynamicImport, entryRootPath);
          if (rootPath) {
            try {
              const pkgJson = await esm.readJSON(`${rootPath}/package.json`);
              version = pkgJson.version;
        ...
        depsToOptimize.set(dynamicImport, {
          exports: ["*"],
          rootPath: rootPath ? chunkITC6JDLC_cjs.slash(rootPath) : null,
          isWorkspace: false,
          version
        });
      }
    ...
    moduleParsed(info) {
      ...
      for (const importedId of info.importedIds) {
        for (const external of externals) {
          if (chunkITC6JDLC_cjs.isDependencyPartOfPackage(importedId, external)) {
        ...
        if (chunk.type === "chunk") {
          for (const [external, resolvedFrom] of importMap) {
            if (chunk.moduleIds.includes(resolvedFrom)) {
              const fullPath = url.pathToFileURL(chunkITC6JDLC_cjs.slash(path.join(projectRoot, fileName))).toString();
              const innerMap = resolveMap.get(fullPath) || /* @__PURE__ */ new Map();
              innerMap.set(external, url.pathToFileURL(chunkITC6JDLC_cjs.slash(resolvedFrom)).toString());
...
function prepareEntryFileName(name, rootDir) {
  return chunkITC6JDLC_cjs.rollupSafeName(name, rootDir);
}
function createVirtualDependencies(depsToOptimize, {
  ...
  if (isDev || externalsPreset) {
    for (const [dep, { isWorkspace, rootPath }] of depsToOptimize.entries()) {
      ...
      const entryName = prepareEntryFileName(chunkITC6JDLC_cjs.getCompiledDepCachePath(rootPath, fileName), rootDir);
      fileNameToDependencyMap.set(entryName, dep);
      optimizedDependencyEntries.set(dep, {
        ...currentDepPath,
        name: entryName
...
async function getInputPlugins2(virtualDependencies, {
  ...
  for (const pkg2 of transpilePackages) {
    const dir = await chunkE4SQOERA_cjs.getPackageRootPath(pkg2);
    if (dir) {
      transpilePackagesMap.set(pkg2, chunkITC6JDLC_cjs.slash(dir));
    } else {
  ...
  return [
    ...
    transpilePackagesMap.size ? chunkE4SQOERA_cjs.esbuild({
      format: "esm",
      include: [
    ...
    chunkFileNames: (chunkInfo) => {
      if (noBundling) {
        ...
        for (const moduleId of chunkInfo.moduleIds) {
          const normalized = chunkITC6JDLC_cjs.slash(moduleId);
          for (const [pkgName, pkgInfo] of workspaceMap.entries()) {
            const location = chunkITC6JDLC_cjs.slash(pkgInfo.location);
        ...
        if (importedFromPackages.size === 1) {
          ...
          return prepareEntryFileName(chunkITC6JDLC_cjs.getCompiledDepCachePath(workspaceLocation, "[name].mjs"), rootDir);
        }
      }
...
function findExternalImporter(module, external, allOutputs) {
  ...
  for (const id of module.imports) {
    if (chunkITC6JDLC_cjs.isDependencyPartOfPackage(id, external)) {
      return module;
    } else {
      if (id.endsWith(".mjs")) {
...
If you think your configuration is valid, please open an issue.`);
  ...
  for (const entry of entries) {
    ...
    for (const [dep, metadata] of analyzeResult.dependencies.entries()) {
      const isPartOfExternals = allExternals.some((external) => chunkITC6JDLC_cjs.isDependencyPartOfPackage(dep, external));
  ...
  const relativeWorkspaceFolderPaths = Array.from(workspaceMap.values()).map(
    (pkgInfo) => chunkITC6JDLC_cjs.slash(path.relative(workspaceRoot || projectRoot, pkgInfo.location))
  );
  ...
  for (const o of output) {
    ...
    for (const i of o.imports) {
      if (chunkITC6JDLC_cjs.isBuiltinModule(i)) {
        continue;
      }
      ...
      const pkgName = chunkITC6JDLC_cjs.getPackageName(i);
      if (pkgName && !allUsedExternals.has(pkgName)) {
...
//# sourceMappingURL=chunk-SAAWBL2Q.cjs.map
//# sourceMappingURL=chunk-SAAWBL2Q.cjs.map