...
var Bundler = class extends bundler.MastraBundler {
  ...
  createBundler(inputOptions, outputOptions) {
    return chunkR2SIOO54_cjs.createBundler(inputOptions, outputOptions);
  }
  async getUserBundlerOptions(mastraEntryFile, outputDirectory) {
    ...
    try {
      const bundlerOptions = await chunkK7J2W7DJ_cjs.getBundlerOptions(mastraEntryFile, outputDirectory);
  ...
  async analyze(entry, mastraFile, outputDirectory) {
    return await chunkSAAWBL2Q_cjs.analyzeBundle(
      [].concat(entry),
      mastraFile,
      {
  ...
  async getBundlerOptions(serverFile, mastraEntryFile, analyzedBundleInfo, toolsPaths, { enableSourcemap, enableEsmShim, externals }) {
    const { workspaceRoot } = await chunkSAAWBL2Q_cjs.getWorkspaceInformation({ mastraEntryFile });
  ...
  getAllToolPaths(mastraDir, toolsPaths = []) {
    const normalizedMastraDir = chunkITC6JDLC_cjs.slash(mastraDir);
  ...
  async _bundle(serverFile, mastraEntryFile, {
    ...
    try {
      ...
      analyzedBundleInfo = await chunkSAAWBL2Q_cjs.analyzeBundle(
    ...
    for (const [dep, depInfo] of analyzedBundleInfo.externalDependencies) {
      ...
      try {
        let rootPath = await chunkE4SQOERA_cjs.getPackageRootPath(dep, projectRoot);
        if (!rootPath) {
          rootPath = await chunkE4SQOERA_cjs.getPackageRootPath(dep, undefined);
        }
        if (rootPath) {
...
//# sourceMappingURL=chunk-SYOOUXQR.cjs.map
//# sourceMappingURL=chunk-SYOOUXQR.cjs.map